...
async def get_walkthrough_audio(walkthrough_id: str, authorization: str = Header(None)):
    ...
    if not audio:
        # Audio is still being generated – return 202 with a status payload
        from fastapi.responses import JSONResponse
        return JSONResponse(
            status_code=202,
            content={"status": "generating", "message": "Audio is being generated by ElevenLabs…"},
        )
...
async def stream_walkthrough_audio(walkthrough_id: str, authorization: str = Header(None)):
    ...
    if walkthrough_id in audio_bytes_store:
        return Response(
            content=audio_bytes_store[walkthrough_id],
